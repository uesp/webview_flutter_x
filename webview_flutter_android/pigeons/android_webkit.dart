...
abstract class WebView extends View {
  ...
  /// Native pan / drag scroll gesture (start / update / end / cancel).
  ///
  /// Positions and deltas are in logical pixels. [velocityX] / [velocityY] are
  /// only meaningful on [ScrollGesturePhase.end] (px/s).
  late void Function(
    ScrollGesturePhase eventType,
    double timestamp,
    double globalX,
    double globalY,
    double localX,
  ...
  /// Enables or disables the native scroll-gesture listener.
  ///
  /// When [consume] is true, touch moves claimed as scrolls are not delivered
  /// to the WebView's own scroller.
  void setScrollGestureListening(bool enabled, bool consume);
