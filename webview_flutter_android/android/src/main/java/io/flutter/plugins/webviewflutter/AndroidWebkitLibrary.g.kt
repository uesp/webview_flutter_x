...
private class AndroidWebkitLibraryPigeonProxyApiBaseCodec(val registrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) : AndroidWebkitLibraryPigeonCodec() {
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?) {
    if (value is Boolean || value is ByteArray || value is Double || value is DoubleArray || value is FloatArray || value is Int || value is IntArray || value is List<*> || value is Long || value is LongArray || value is Map<*, *> || value is String || value is FileChooserMode || value is ConsoleMessageLevel || value is OverScrollMode || value is SslErrorType || value is MixedContentMode || value is WindowInsetsType || value is ScrollGesturePhase || value == null) {
...
enum class ScrollGesturePhase(val raw: Int) {
  ...
  companion object {
    fun ofRaw(raw: Int): ScrollGesturePhase? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
private open class AndroidWebkitLibraryPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      135.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          ScrollGesturePhase.ofRaw(it.toInt())
        }
      }
      else -> super.readValueOfType(type, buffer)
    }
...
abstract class PigeonApiWebView(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
   ...
   */
  abstract fun setScrollGestureListening(pigeon_instance: android.webkit.WebView, enabled: Boolean, consume: Boolean)
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiWebView?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebView.setScrollGestureListening", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebView
            val enabledArg = args[1] as Boolean
            val consumeArg = args[2] as Boolean
            val wrapped: List<Any?> = try {
              api.setScrollGestureListening(pigeon_instanceArg, enabledArg, consumeArg)
              listOf(null)
            } catch (exception: Throwable) {
              AndroidWebkitLibraryPigeonUtils.wrapError(exception)
            }
  ...
  fun onScrollChanged(pigeon_instanceArg: android.webkit.WebView, leftArg: Long, topArg: Long, oldLeftArg: Long, oldTopArg: Long, callback: (Result<Unit>) -> Unit)
{
   ...
   */
  fun onScrollGesture(pigeon_instanceArg: android.webkit.WebView, eventTypeArg: ScrollGesturePhase, timestampArg: Double, globalXArg: Double, globalYArg: Double, localXArg: Double, localYArg: Double, deltaXArg: Double, deltaYArg: Double, velocityXArg: Double, velocityYArg: Double, callback: (Result<Unit>) -> Unit)
{
    if (pigeonRegistrar.ignoreCallsToDart) {
      callback(
          Result.failure(
      ...
      callback(
          Result.failure(
              AndroidWebKitError("missing-instance-error", "Callback to `WebView.onScrollGesture` failed because native instance was not in the instance manager.", "")))
      return
    }
    val binaryMessenger = pigeonRegistrar.binaryMessenger
    val codec = pigeonRegistrar.codec
    val channelName = "dev.flutter.pigeon.webview_flutter_android.WebView.onScrollGesture"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(pigeon_instanceArg, eventTypeArg, timestampArg, globalXArg, globalYArg, localXArg, localYArg, deltaXArg, deltaYArg, velocityXArg, velocityYArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(AndroidWebKitError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
