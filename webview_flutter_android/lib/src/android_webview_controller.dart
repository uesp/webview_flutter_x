...
class AndroidWebViewController extends PlatformWebViewController {
  ...
  late final android_webview.WebView _webView = android_webview.WebView(
    ...
    onScrollGesture: withWeakReferenceTo(this, (
      WeakReference<AndroidWebViewController> weakReference,
    ) {
      return (
        _,
        android_webview.ScrollGesturePhase eventType,
        double timestamp,
        double globalX,
        double globalY,
        double localX,
        ...
        controller._onScrollGestureCallback?.call(
          ScrollGestureEvent(
            eventType: controller._toScrollGesturePhase(eventType),
            timestamp: timestamp,
  ...
  /// Sets a listener for native Android pan / drag scroll gestures.
  ///
  /// When [consume] is true, scroll moves are not delivered to the WebView's
  /// own scroller so a Flutter-owned scroll can drive the document.
  Future<void> setOnScrollGesture(
    void Function(ScrollGestureEvent event)? callback, {
    bool consume = false,
  }) {
    _onScrollGestureCallback = callback;
    return _webView.setScrollGestureListening(callback != null, consume);
  }

  ScrollGesturePhase _toScrollGesturePhase(
    android_webview.ScrollGesturePhase phase,
  ) {
    return switch (phase) {
      android_webview.ScrollGesturePhase.start => ScrollGesturePhase.start,
      android_webview.ScrollGesturePhase.update => ScrollGesturePhase.update,
      android_webview.ScrollGesturePhase.end => ScrollGesturePhase.end,
      android_webview.ScrollGesturePhase.cancel => ScrollGesturePhase.cancel,
    };
  }
