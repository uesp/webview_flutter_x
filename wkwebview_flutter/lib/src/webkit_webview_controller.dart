...
class WebKitWebViewController extends PlatformWebViewController {
  ...
  UIScrollViewDelegate? _uiScrollGestureDelegate;
  ...
  void Function(ScrollGestureEvent event)? _onScrollGestureCallback;

  /// Recent scroll-gesture deltas with timestamps (ms) for velocity estimation
  /// (macOS wheel path; iOS supplies velocity from the pan recognizer on end).
  final List<({Offset delta, double timeMs})> _scrollGestureSamples =
  ...
  /// Sets a listener for native scroll gestures (macOS wheel / iOS pan).
  ///
  /// On macOS the `NSEvent` monitor is scoped to the web view itself.
  /// On iOS a `UIPanGestureRecognizer` is attached to `WKWebView.scrollView`.
  ///
  /// When [consume] is true, native document scrolling is suppressed so a
  /// Flutter-owned scroll can drive the content (iOS: `isScrollEnabled = false`;
  /// macOS: wheel events are eaten by the monitor).
  Future<void> setOnScrollGesture(
    void Function(ScrollGestureEvent event)? callback, {
    bool consume = false,
  }) {
    _onScrollGestureCallback = callback;
    if (defaultTargetPlatform == TargetPlatform.macOS) {
      return _setMacOnScrollGesture(callback, consume: consume);
    }
    if (defaultTargetPlatform == TargetPlatform.iOS) {
      return _setIOSOnScrollGesture(callback, consume: consume);
    }
    throw UnimplementedError(
      'setOnScrollGesture is only implemented on iOS and macOS',
    );
  }

  ScrollGesturePhase _toScrollGesturePhase(FWFNSScrollWheelPhase phase) {
    return switch (phase) {
      FWFNSScrollWheelPhase.start => ScrollGesturePhase.start,
      FWFNSScrollWheelPhase.update => ScrollGesturePhase.update,
      FWFNSScrollWheelPhase.end => ScrollGesturePhase.end,
      FWFNSScrollWheelPhase.cancel => ScrollGesturePhase.cancel,
    };
  }

  /// Window (ms) of recent deltas used to estimate scroll-gesture velocity.
  static const double _scrollGestureVelocityWindowMs = 100;

  /// Records the [delta] sampled at [timeMs] for [phase] and returns the
  /// gesture velocity (px/s). Velocity is only computed on
  /// [ScrollGesturePhase.end]; other phases return [Offset.zero].
  Offset _trackScrollGestureVelocity(
    ScrollGesturePhase phase,
    Offset delta,
    double timeMs,
  ) {
    switch (phase) {
      case ScrollGesturePhase.start:
        _scrollGestureSamples.clear();
        _recordScrollGestureSample(delta, timeMs);
      case ScrollGesturePhase.update:
        _recordScrollGestureSample(delta, timeMs);
      case ScrollGesturePhase.end:
        if (delta != Offset.zero) {
          _recordScrollGestureSample(delta, timeMs);
        }
        final Offset velocity = _computeScrollGestureVelocity();
        _scrollGestureSamples.clear();
        return velocity;
      case ScrollGesturePhase.cancel:
        _scrollGestureSamples.clear();
    }
    return Offset.zero;
  }
  ...
  /// [_scrollGestureVelocityWindowMs].
  void _recordScrollGestureSample(Offset delta, double timeMs) {
    _scrollGestureSamples.add((delta: delta, timeMs: timeMs));
    while (_scrollGestureSamples.length > 1 &&
        timeMs - _scrollGestureSamples.first.timeMs >
            _scrollGestureVelocityWindowMs) {
      _scrollGestureSamples.removeAt(0);
    }
  }

  /// Estimates scroll-gesture velocity (px/s) from the recent sample window.
  Offset _computeScrollGestureVelocity() {
    if (_scrollGestureSamples.length < 2) {
      return Offset.zero;
    }
    final double spanMs =
        _scrollGestureSamples.last.timeMs - _scrollGestureSamples.first.timeMs;
    if (spanMs <= 0) {
      return Offset.zero;
    }
    Offset displacement = Offset.zero;
    for (int i = 1; i < _scrollGestureSamples.length; i++) {
      displacement += _scrollGestureSamples[i].delta;
    }
    return displacement * (1000 / spanMs);
  }

  Future<void> _setMacOnScrollGesture(
    void Function(ScrollGestureEvent event)? callback, {
    ...
    _nsScrollWheelDelegate = FWFNSScrollViewDelegate(
      scrollWheel: (
        ...
        final ScrollGesturePhase phase = controller._toScrollGesturePhase(
          eventType,
        );
        final Offset delta = Offset(deltaX, deltaY);
        final Offset velocity = controller._trackScrollGestureVelocity(
          phase,
          delta,
          timestamp * 1000,
        );
        controller._onScrollGestureCallback?.call(
          ScrollGestureEvent(
            eventType: phase,
            timestamp: timestamp,
  ...
  Future<void> _setIOSOnScrollGesture(
    void Function(ScrollGestureEvent event)? callback, {
    required bool consume,
  }) async {
    if (callback == null) {
      _uiScrollGestureDelegate = null;
      await _webView.setScrollGestureDelegate(null, consume: false);
      return;
    }

    final WeakReference<WebKitWebViewController> weakThis =
        WeakReference<WebKitWebViewController>(this);
    _uiScrollGestureDelegate = UIScrollViewDelegate(
      scrollGesture: (
        _,
        _,
        FWFNSScrollWheelPhase eventType,
        ...
        final Offset delta = Offset(deltaX, deltaY);
        // Prefer pan-recognizer velocity on end; otherwise estimate from samples.
        final Offset velocity = phase == ScrollGesturePhase.end
            ? Offset(velocityX, velocityY)
            : controller._trackScrollGestureVelocity(
                phase,
                delta,
                timestamp * 1000,
              );
        if (phase == ScrollGesturePhase.end) {
          controller._scrollGestureSamples.clear();
        }
        controller._onScrollGestureCallback?.call(
          ScrollGestureEvent(
            eventType: phase,
    ...
    await _webView.setScrollGestureDelegate(
      _uiScrollGestureDelegate,
      consume: consume,
    );
  }
