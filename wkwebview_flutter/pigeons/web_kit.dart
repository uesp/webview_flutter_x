...
abstract class UIScrollView extends UIView {
  ...
  ///
  /// When false, the scroll view does not respond to user touch scrolling.
...
abstract class UIViewWKWebView extends UIView implements WKWebView {
  ...
  /// Installs a scroll-gesture delegate that receives native pan events.
  ///
  /// When [consume] is true, native [UIScrollView] scrolling is disabled while
  /// the delegate is attached so a Flutter-owned scroll can drive the document.
  void setScrollGestureDelegate(
...
abstract class UIScrollViewDelegate extends NSObject {
  ...
  /// Tells the delegate when a native pan scroll gesture occurs on the
  /// scroll view.
  ///
  /// [velocityX] / [velocityY] are only meaningful on [FWFNSScrollWheelPhase.end]
  /// (points per second from the pan recognizer); otherwise zero.
  void Function(
