...
import WebKit
  ...
  /// Scroll-gesture pan monitors keyed by the `WKWebView` they observe.
  private var iosWebViewScrollGestureDelegates:
    [ObjectIdentifier: ScrollViewDelegateImpl] = [:]
#endif
#if os(macOS)
  import AppKit
  ...
  PigeonApiDelegateNSViewWKWebView
{
  func getUIViewWKWebViewAPI(_ api: PigeonApiNSViewWKWebView) -> PigeonApiUIViewWKWebView {
    ...
    func setScrollGestureDelegate(
      ...
      iosWebViewScrollGestureDelegates[viewId]?.detachScrollGesture()
      iosWebViewScrollGestureDelegates.removeValue(forKey: viewId)
      guard let impl = delegate as? ScrollViewDelegateImpl else { return }
      iosWebViewScrollGestureDelegates[viewId] = impl
      impl.attachScrollGesture(to: pigeonInstance, consume: consume)
    }
  #endif
